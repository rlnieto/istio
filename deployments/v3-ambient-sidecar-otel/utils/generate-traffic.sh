...
#  curl -s http://$HOSTNAME/productpage > /dev/null
  http_code_page=$(curl -sk -w "%{http_code}" -o /dev/null http://$HOSTNAME/sidecar/productpage)
  sleep 0.5
  http_code_json=$(curl -sk -w "%{http_code}" -o /dev/null http://$HOSTNAME/sidecar/api/v1/products)
  echo -e "(${http_code_page}/${http_code_json})"

  http_code_page=$(curl -sk -w "%{http_code}" -o /dev/null http://$HOSTNAME/ambient/productpage)
  sleep 0.5
  http_code_json=$(curl -sk -w "%{http_code}" -o /dev/null http://$HOSTNAME/ambient/api/v1/products)
  echo -e "               (${http_code_page}/${http_code_json})"
